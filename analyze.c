...
/***
 * いずれの解析関数も
 * ・構造体のサイズ分のデータがあるかどうかのチェック
 * ・データポインタを構造体にキャストして代入
 * ・表示用関数の呼び出し
 ***/
...
int AnalyzeIp(u_char *data, int size)
{
    ...
    /***
     * IP ヘッダには不定長のオプションが存在する。ここではその分のポインタを進める。
     * （ IP ヘッダの最後に可変長のオプションとパディングが存在する）
    ***/
...
int AnalyzeIpv6(u_char *data, int size)
{
    ...
    if(ip6->ip6_nxt == IPPROTO_ICMPV6){
        ...
        if(checkIP6DATAchecksum(ip6, ptr, len) == 0){
            fprintf(stderr, "bad icmp6 chechsum\n");
            return(-1);
        }
        PrintIcmp6(ip6, stdout);
    }
    else if(ip6->ip6_nxt == IPPROTO_TCP){
        len = ntohs(ip6->ip6_plen);
        if(checkIP6DATAchecksum(ip6, ptr, len) == 0){
            fprintf(stderr, "bad tcp6 chechsum\n");
            return(-1);
        }
        AnalyzeTcp(ptr, lest);
    }
        ...
        len = ntohs(ip6->ip6_plen);
        if(checkIP6DATAchecksum(ip6, ptr, len) == 0){
            fprintf(stderr, "bad udp6 chechsum\n");
            return(-1);
        }
        AnalyzeUdp(ptr, lest);
    }
